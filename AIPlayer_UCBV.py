...
class MCTS(object):
    def __init__(self, board, color, max_iter):
    ...
    def best_child(self,node,Cp):
        ...
        for sub_node in node.children.keys():
            if sub_node.visit_times < 2:
                return sub_node
            # 如果是轮到我方下棋，那么胜率正常计算
            if node.color == self.mycolor:
                value = sub_node.quality_value/sub_node.visit_times+ math.sqrt(2 * np.var(sub_node.rewards) * math.log(node.visit_times)/sub_node.visit_times) +Cp*math.log(node.visit_times)/sub_node.visit_times
            # 如果是轮到对方下棋，那么胜率要反过来
            else:
                value = 1 - (sub_node.quality_value/sub_node.visit_times+ math.sqrt(2 * np.var(sub_node.rewards) * math.log(node.visit_times)/sub_node.visit_times) +Cp*math.log(node.visit_times)/sub_node.visit_times)
            if value>max_value:
                best_node = sub_node
                max_value = value
        return best_node
